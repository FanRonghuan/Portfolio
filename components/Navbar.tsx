...
const SpotlightLink: React.FC<{ text: string; href: string; isScrolled: boolean; onClick?: (e: React.MouseEvent) => void }> = ({ text, href, isScrolled, onClick }) => {
    ...
    return (
        <Magnetic>
            <motion.a
                ...
                className={`relative px-4 py-2 text-[10px] font-bold tracking-widest cursor-pointer block group overflow-hidden rounded-lg transition-colors duration-500 ${isScrolled ? 'text-gray-600' : 'text-white/70'}`}
...
const Navbar: React.FC = () => {
  ...
  // Threshold increased to allow more video visibility before navbar transition
          ...
          <div className="flex items-center space-x-4">
            ...
            <Magnetic>
                <motion.a
                    ...
                    whileTap={{ scale: 0.95 }}
                    className={`ml-4 px-6 py-2 text-[10px] font-bold tracking-widest border rounded-full backdrop-blur-md transition-all block ${isScrolled ? 'text-gray-600 border-black/10 bg-white/30 hover:bg-white/50 hover:text-black' : 'text-white/70 border-white/20 bg-white/10 hover:bg-white/20 hover:text-white'}`}
                >
                    <span>CONTACT</span>
                </motion.a>
            </Magnetic>
