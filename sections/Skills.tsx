...
const RadarScan: React.FC<{ color: string, className?: string }> = ({ color, className }) => {
    return (
...
const SignalBars: React.FC<{ percent: number, color: string }> = ({ percent, color }) => {
    const barCount = 5;
...
const GlassSkillCard: React.FC<{ 
    ...
    return (
        ...
        >
            ...
            <div 
                className="absolute inset-0 rounded-[2rem] bg-deep-space/40 border border-white/10 pointer-events-none"
                style={{ 
            ...
            />
            <div 
                className="absolute inset-[-1px] rounded-[2rem] border border-white/5 pointer-events-none"
                style={{ transform: 'translateZ(-8px)' }}
            />

            {/* --- MAIN GLASS FACE --- */}
            <div className="absolute inset-0 rounded-[2rem] shadow-[0_15px_30px_-5px_rgba(0,0,0,0.5)]" style={{ transformStyle: 'preserve-3d' }}>
                    ...
                    <div className="relative z-20 p-6 flex items-center gap-6 h-full">
                        ...
                        <div className="flex-grow flex flex-col justify-center">
                            <div className="flex justify-between items-end mb-2">
                                <h3 className="text-2xl font-space-grotesk font-bold text-white tracking-tight">{skill.title}</h3>
                                <SignalBars percent={skill.percent} color={skill.color} />
                            </div>

                            <div className="text-[10px] font-jetbrains-mono text-white/30 uppercase tracking-widest mt-1">
                                {skill.tags}
                            </div>
                        </div>
                    </div>
    ...
    return (
        <Magnetic strength={20}>
            <motion.div
                ...
                // Original Size
                className={`${SOFTWARE_ICON_CLASS} rounded-2xl bg-deep-space/40 backdrop-blur-xl border border-white/10 shadow-[0_10px_20px_-5px_rgba(0,0,0,0.5),0_4px_0_rgba(255,255,255,0.02)] flex items-center justify-center cursor-pointer group relative overflow-hidden will-change-transform`}
            ...
            >
                ...
                <AnimatePresence>
                    ...
                    {hoveredSkill && (
                        ...
                        >
                            <div className="w-full h-full bg-black/80 backdrop-blur-xl p-3 rounded-[2rem] shadow-2xl border border-white/10">
                                <div className="w-full h-full rounded-[1.5rem] overflow-hidden bg-white/5 relative">
                                    ...
                                    <div className="absolute bottom-4 left-4 bg-black/60 backdrop-blur px-4 py-2 rounded-xl border border-white/10">
                                        <p className="font-albert-black text-sm text-white">{hoveredSkill.previewText || 'DESIGN WORK PREVIEW'}</p>
                                    </div>
                                </div>
                            </div>
                        </motion.div>
                    ...
                    {hoveredSoftware && (
                        ...
                        >
                            <div className="w-full h-full bg-black/40 backdrop-blur-xl border border-white/10 p-8 rounded-[2rem] shadow-2xl flex items-center justify-center">
                                ...
                                <div className="absolute bottom-4 text-xs font-mono text-white/40">
