    ...
    return (
        ...
        >
            <div className="relative w-full h-full group">
                ...
                <div className="absolute inset-0 rounded-[2rem] bg-deep-space border-2 border-white/10 overflow-hidden shadow-[0_10px_40px_rgba(0,0,0,0.6)] transition-all duration-300 group-hover:border-white/30">
                    {/* Project Image */}
                    <div className="absolute inset-0 overflow-hidden opacity-80 group-hover:opacity-100 transition-opacity duration-500">
                ...
                <div className="absolute top-4 left-4 z-20">
                    <motion.div 
                        className="w-10 h-10 rounded-full bg-black/40 backdrop-blur-md border border-white/20 flex items-center justify-center text-white font-albert-black text-sm shadow-xl"
...
const SignalMonitor: React.FC<{ scrollVal: number, mouseVal: number }> = ({ scrollVal, mouseVal }) => (
    <div className="fixed top-24 right-10 z-[70] font-mono text-[10px] text-pulse-orange bg-deep-space/80 backdrop-blur-md px-4 py-3 rounded-xl border border-pulse-orange/30 pointer-events-none tracking-widest flex flex-col gap-2 shadow-2xl">
        <div className="flex items-center gap-2 mb-1">
            <div className="w-1.5 h-1.5 rounded-full bg-pulse-orange animate-pulse" />
            <span className="font-bold">SIGNAL MONITOR</span>
        </div>
        <span className="flex justify-between gap-6 opacity-60"><span>PULSE_Y:</span> <span>{scrollVal}</span></span>
        <span className="flex justify-between gap-6 opacity-60"><span>PULSE_X:</span> <span>{mouseVal}</span></span>
    </div>
);
    ...
    if (project?.layout === 'pdf' && project.pdfUrl) {
        return (
            <div className="w-full h-full bg-black overflow-hidden relative">
                <SignalMonitor scrollVal={0} mouseVal={0} />
                <iframe 
    ...
    return (
        ...
        <div className="relative w-full h-full bg-deep-space">
            ...
            >
                ...
                <SignalMonitor scrollVal={scrollVal} mouseVal={mouseVal} />
